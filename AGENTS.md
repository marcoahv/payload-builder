...
Payload CMS 3.x + Next.js 16, MongoDB, npm.

- Dev server: `npm run dev` (http://localhost:3000)
- Build: `npm run build`
- Production build + start: `npm run start` (runs the production build, then
  `next start`)
- Lint: `npm run lint`
- Payload CLI: `npm run payload -- <command>`
- Regenerate types after a schema change: `npm run generate:types`
- Regenerate the admin import map after adding/moving a custom component:
  `npm run generate:importmap`
- Unit/integration tests (Vitest): `npm run test:int`
- Browser tests (Playwright): `npm run test:e2e`
- Both: `npm run test`

Test runners are configured but no test files exist yet - `tests/int/` is
created by the first logic-bearing step that needs one. No combined `Verify`
command or GitHub Actions workflow exists yet; run `/ci` or `$ci` to set one
up when you want automatic PR checks.
